;-------------------------------------------------------------------------------------
;This is a modified version the modified version so it works with asm6.  The header
;is made automatically but you need the chr data.  It should be a file named 
;smb_chr.chr.
;    -- SpiderDave
...
;.segment "INESHDR"
  ...
  .db 2                                         ;  PRG ROM size in 16384 byte units
  .db 1                                         ;      CHR
  .db $01                                       ;  mirroring type and mapper number lower nibble
...
WriteBottomStatusLine:
      jsr AddToScore+4        ;honestly not sure why it JSRs here
      ...
      ldy IntervalTimerControl ;next framerule value
      nop                     ;don't increment for proper number display
      tya
...
UpdateTopScore:
			rts
Sockvalue:	.db $0c, $08, $0d, $07	;Really only useful for 1-1 (0C8D70)

TopScoreCheck:
              ldx #$03
Sockfolder:	  lda ScoreAndCoinDisplay+2, X
			  cmp Sockvalue,X
			  bne NoStarTimer
			  dex
			  bpl Sockfolder
...
GetScoreDiff: sta StarInvincibleTimer
			  rts
...
PlayerSubs: jsr CoinPoints              ;move the screen if necessary
...
AddToScore:
	  jsr TopScoreCheck
      jsr ScrollHandler  ;update the score internally with value in digit modifier
...
;frenzy object jump table
      ...
      .dw InitBowserFlame
      .dw NoFWork ;don't init fireworks
...
RunStarFlagObj:
      ...
      .dw StarFlagExit
      .dw IncrementSFTask1 ;don't try to do fireworks
...
ELPGive: nop
		 nop
		 nop 					;award 50 points per game timer interval
